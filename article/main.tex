...
base (\textasciitilde750~GB), a amostra piloto foi extraída em duas rodadas por um
Kaggle Notebook (dado já residente no servidor, sem download local do conjunto
completo): 22 exames --- 11 positivos e 11 negativos, sementes fixas (42 e 43) ---
com 6--15 cortes aleatórios por exame (variável por rodada, limitado pela
...
execução real de \texttt{scripts/run\_experiment.py} sobre os 22 exames
\textbf{reais} descritos em \S3.1, não sintéticos, com ajuste de hiperparâmetros em
validação aninhada. Com 11 exames por classe, as métricas ainda têm variância
relevante e \textbf{não devem ser lidas como benchmark definitivo}, mas já são
suficientes para uma leitura comparativa inicial entre famílias de modelos.

\begin{table}[h]
\centering
\caption{Comparação descritor~$\times$~modelo (média~$\pm$~desvio, 5 dobras, 22 exames reais: 11 positivos + 11 negativos).}
...
\midrule
Regressão logística    & 0,783 $\pm$ 0,194 & 0,840 $\pm$ 0,183 & 0,633 $\pm$ 0,100 \\
MLP raso                & 0,767 $\pm$ 0,122 & 0,857 $\pm$ 0,081 & 0,700 $\pm$ 0,194 \\
$k$-NN                   & 0,600 $\pm$ 0,276 & 0,723 $\pm$ 0,225 & 0,633 $\pm$ 0,251 \\
SVM (RBF)                 & 0,600 $\pm$ 0,122 & 0,686 $\pm$ 0,112 & 0,500 $\pm$ 0,158 \\
Random Forest              & 0,575 $\pm$ 0,350 & 0,701 $\pm$ 0,215 & 0,500 $\pm$ 0,279 \\
Baseline trivial            & 0,500 $\pm$ 0,000 & 0,500 $\pm$ 0,063 & 0,500 $\pm$ 0,000 \\
XGBoost                      & 0,300 $\pm$ 0,292 & 0,549 $\pm$ 0,204 & 0,367 $\pm$ 0,145 \\
SVM (linear)                  & 0,267 $\pm$ 0,162 & 0,514 $\pm$ 0,127 & 0,467 $\pm$ 0,251 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.32\linewidth]{figures/roc_logreg.png}
\caption{Curva ROC fora-de-dobra da regressão logística (melhor AUC-ROC médio), $N{=}22$.}
\label{fig:roc}
\end{figure}

Regressão logística e MLP raso ficaram acima do baseline trivial (0,500); XGBoost
e SVM linear ficaram \emph{abaixo} --- com $N{=}22$ para 8 modelos, alguns
superajustam às centenas de características extraídas. Matriz de confusão
fora-de-dobra da regressão logística: 6~VN, 5~FP,
3~FN, 8~VP (sensibilidade~=~72,7\%, especificidade~=~54,5\%). \textbf{Erro
qualitativo:} os 8 erros distribuem-se de forma equilibrada entre falsos positivos
e negativos, sem exame específico como outlier de score extremo --- diferente do
piloto $N{=}6$, consistente com a hipótese de que mais dados diluem erros
idiossincráticos de exames individuais.
...
\textbf{real} de 22 exames (ampliada a partir de um piloto inicial de 6). Regressão
logística e MLP raso superaram consistentemente o baseline trivial (AUC-ROC~0,78
e~0,77 vs.\ 0,50); XGBoost e SVM linear ficaram abaixo --- com $N{=}22$ para 8
modelos, superajuste ainda é esperado. A ausência de segmentação vascular/pulmonar
supervisionada é a segunda limitação relevante. Trabalho futuro: (i) ampliar a
amostra para centenas de exames; (ii) seleção de características mais agressiva
para reduzir superajuste; (iii) comparar contra aprendizado profundo (ex.: réplica
do PENet \cite{huang2020penet}).
